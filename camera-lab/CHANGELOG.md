...
- **GitHub Pages:** After Jekyll, workflow runs `sudo chown -R "$USER:$USER" _site` before merging `camera-lab/dist` (Docker-built `_site` was root-owned → `Permission denied` on `mkdir`).
